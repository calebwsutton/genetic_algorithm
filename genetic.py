...
def crossover(parent1, parent2):
	...
	while (parent1['alleles'][cross_point] == 0 and parent1['alleles'][cross_point + 1] == 0): # generate a new cross point as long as 
		cross_point = random.randint(0, len(parent1['alleles'] - 2))
	
	offspring = parent1[:cross_point] + parent2[cross_point+1:]
	mutate(offspring)
			...
			if chromosome['alleles'][i] == 0:   # if the allele is a 0 we can just flip it to a 1 now worries
				...
				if i == 0 and i + 1 < len(chromosome['alleles']):   # Case: first allele
					if  chromosome['alleles'][i + 1] == 1:          # Only Check the next allele
						...
						return_val = True
				elif i == len(chromosome['alleles']) - 1:           # Case: last allele
					if chromosome['alleles'][i - 1] == 1:           # Only check the previous allele
						chromosome['alleles'][i] = 0
						return_val = True
